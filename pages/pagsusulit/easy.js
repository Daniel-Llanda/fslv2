...
export default function Home() {
  ...
  const [targetLetter, setTargetLetter] = useState("")     // what the user must do next
  const [detectedLetter, setDetectedLetter] = useState("") // optional: what model sees (debug)
  const [tutorText, setTutorText] = useState("")
  const [finished, setFinished] = useState(false)
  const [introOpen, setIntroOpen] = useState(true)

  // ---------- Stage/level state ----------
  const STAGES = useRef(["DOG", "CAT", "HOT", "AIR", "DRY"])
  ...
  // Build the sign list for a word (e.g., "DOG" -> ['d','o','g'] signs)
  ...
  function advanceStageOrFinish() {
    ...
    if (nextIndex < STAGES.current.length) {
      ...
      if (currentResult !== "finished_hard") {
        storage.set("result", "finished_easy")
      }
        ...
        if (
          ...
          if (detected === "thumbs_up" && gamestateRef.current !== "played") {
            ...
            setTutorText(
              ...
              }/${STAGES.current.length}: Make the hand signs for "${STAGES.current[stageIndexRef.current]
              }"`
            )
            ...
            if (current) {
              // Always show the REQUIRED letter (what the user must do next)
      ...
      <Box bgColor="#5784BA">
        <Container centerContent maxW="xl" height="100vh" pt="0" pb="0">
          ...
          <Box id="webcam-container" position="relative">
            ...
            {/* Overlay that shows the REQUIRED letter */}
              ...
              >
                ...
                <Box display="flex" gap="8px" justifyContent="center" alignItems="center">
                  {STAGES.current[stageIndexRef.current]
                    ...
                    .split("")
                    .map((ch, i) => {
                      // completed letters are green
                      const isDone = i < currentSignRef.current
                      // current letter gets a highlight color
                      const isCurrent = i === currentSignRef.current
                      ...
                      return (
                        <Text
                          ...
                          fontSize="6xl"
                          fontWeight="bold"
                          // green when done, yellow highlight for current, dim white for pending
                          color={
                            isDone
                              ? "green.300"
                              : isCurrent
                          ...
                          // small visual cue for current letter
        ...
        {introOpen && (
          ...
          >
            ...
            >
              ...
              <Text mb={3}>
                • This activity has {STAGES.current.length} stages:{" "}
                {STAGES.current.join(" → ")}.
              </Text>
              <Text mb={3}>
