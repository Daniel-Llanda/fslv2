...
const storage = {
  set(key, value) {
    try { window.localStorage.setItem(key, JSON.stringify(value)) } catch (err) { console.error("localStorage set failed:", err) }
  },
  ...
  remove(key) {
    try { window.localStorage.removeItem(key) } catch (err) { console.error("localStorage remove failed:", err) }
  }
}
...
export default function Home() {
  ...
  const [targetLetter, setTargetLetter] = useState("")     // what user must do next
  const [detectedLetter, setDetectedLetter] = useState("") // what model currently sees (debug)

  // ---------- Stage/level state ----------
  // HARD: 7-letter words
  const STAGES = useRef(["BALANCE", "VICTORY", "FOREVER", "KINGDOM", "RADIANT"])
  ...
  async function runHandpose() {
    ...
    intervalRef.current = setInterval(() => { detect(net) }, 150)
  }

  // Build the sign list for a word (e.g., "balance" -> letters mapped via Signpass)
  ...
  function loadStage(index) {
    ...
    // show first required letter right away
  ...
  function advanceStageOrFinish() {
    ...
    if (nextIndex < STAGES.current.length) {
      ...
      // Persist completion for HARD
      window.localStorage.setItem("result", "finished_hard")
    ...
    if (
      ...
      if (hand.length > 0) {
        const GE = new fp.GestureEstimator([
          ...
          // You can omit J/Z if motion-based; keeping full set here:
  ...
  // Render the whole word with per-letter coloring
    ...
    <ChakraProvider>
      ...
      <Box bgColor="#5784BA">
        <Container centerContent maxW="xl" height="100vh" pt="0" pb="0">
          ...
          <Box id="webcam-container" position="relative">
            {camState === "on" ? (
              ...
              <div id="webcam" style={{ background: "black", width: 640, height: 480 }} />
            )}
        ...
        <Stack id="start-button" spacing={4} direction="row" align="center" p={4}>
          ...
          <Button colorScheme="gray" onClick={() => (window.location.href = "/pagsusulit")}>
            Back
          </Button>
        </Stack>

        {/* ---------- Intro / Instruction Modal ---------- */}
        {introOpen && (
          <Box
            position="fixed"
            top="0"
            ...
            zIndex={1000}
          >
            <Box bg="white" p={8} rounded="lg" textAlign="left" boxShadow="lg" maxW="lg">
              <Heading size="lg" mb={4}>How to Play</Heading>
              <Text mb={3}>• This activity has {STAGES.current.length} stages of <b>7-letter</b> words: {STAGES.current.join(" → ")}.</Text>
              <Text mb={3}>• When ready, make a <b>👍 thumbs up</b> to start a stage.</Text>
              <Text mb={3}>• Copy the hand signs for each letter of the current word.</Text>
              <Text mb={6}>• Finish all letters to move to the next stage.</Text>
              <Stack direction="row" justify="flex-end">
                <Button variant="ghost" onClick={() => (window.location.href = "/pagsusulit")}>
                  Cancel
                </Button>
        ...
        {finished && (
          <Box
            ...
            zIndex={1000}
          >
            <Box bg="white" p={10} rounded="lg" textAlign="center" boxShadow="lg">
              <Heading size="lg" mb={4}>Task Completed</Heading>
              <Text mb={6}>All stages ({STAGES.current.join(" → ")}) are complete.</Text>
              <Button colorScheme="blue" onClick={() => (window.location.href = "/pagsusulit")}>
                Return
              </Button>
            </Box>
          </Box>
